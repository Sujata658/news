...
const AuthForm = <T extends ZodTypeAny>({ schema, onSubmit, fields }: AuthFormProps<T>) => {
  ...
  return (
    <FormProvider {...formMethods}>
      <form onSubmit={formMethods.handleSubmit(onSubmit)} className="space-y-4 mx-4 ">
        {fields.map((field) => (
          <AuthFormField
            key={field.name}
        ...
        <div className="flex justify-center items-center">
          <Button type="submit" className="bg-primary rounded-[5px] text-lg mt-4">Submit</Button>
        </div>
      </form>
    </FormProvider>
  );
